...
# ----------------------------
# Utilities: Local-LLM JSON IO
# ----------------------------
def _extract_json_substring(text: str) -> Optional[str]:
    """
    Best-effort extraction of a JSON object/array substring from free-form model output.
    Returns the first valid-looking JSON block, or None if not found.
    """
    if not text:
        return None
    ...
    # Fast path: if the whole thing is JSON
    ...
    # Choose the earliest match in the string (if both exist)
    candidates = []
    if obj_match:
...
def llm_generate_text(
    ...
    """
    Generate one completion from a local vLLM engine.
    """
...
def llm_generate_json(
    ...
    """
    Generate JSON (object or array) from local vLLM, with best-effort parsing and retries.
    """
    last_text = ""
    for attempt in range(max_retries):
        text = llm_generate_text(llm, prompt, sampling_params)
        last_text = text
        ...
        try:
            ...
            try:
                json_str2 = re.sub(r",\s*([}\]])", r"\1", json_str)
                return json.loads(json_str2)
            except Exception:
                time.sleep(0.3)
                continue

    raise ValueError(f"Failed to parse JSON from model output after {max_retries} retries. Last output:\n{last_text}")
